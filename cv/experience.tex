...
\textbf{TBank} \hfill October 2026 - Present \\
Senior Research Developer, G.E.R.I.

Optimizing high-scale LLM inference through system design and tuning open-source serving engines.

\textbf{Responsibilities:}

\begin{itemize}[left=0pt, label={--}]
    \item System design for distributed LLM serving infrastructure, including KV cache management.
    \item Tuning open-source LLM inference engines (e.g., SGLang, vLLM, TensorRT-LLM) for high-throughput, low-latency serving.
    \item Low-precision quantization (FP8) and other model-level optimizations for in-house LLMs.
\end{itemize}

\textbf{Achievements:}

\begin{itemize}[left=0pt, label={--}]
    \item Applied \textbf{FP8} quantization with TensorRT-LLM to a custom in-house model, achieving \textbf{2x} throughput per GPU.
    \item Designed and built a KV cache sharing system, lifting per-GPU throughput by up to \textbf{2.2x}.
\end{itemize}

\textbf{Technologies:} \\
Python, PyTorch, TensorRT-LLM, FP8 quantization, CUDA, Triton Server, Docker + Kubernetes, Observability

\newpage

\textbf{Victory Group} \hfill April 2025 - October 2026 \\
